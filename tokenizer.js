...
class Tokenizer {
   ...
   encode(str) { throw new Error("Not implemented."); }
   decode(arr) { throw new Error("Not implemented."); }
}
*/
...
*/
   ...
   encode(str) {
      tq84_log_.enter(`SimpleTokenizer.encode ${str}`);
...
//  console.log(`     bpe_merges[6] = ${bpe_merges[6]}`);
   ...
   encode(text) {
      ...
      // a Uint8Array with the text encoded as UTF8:
         ...
         const new_tokens = this.bpe(bytes_str).map((x) => this.encoder[x]);
